<template>
  <div>
    <div class="selec">
      ...
      <!-- 选座 -->
      <!-- select-position -->
      <v-touch tag="div" class="select-position" v-if="showtimes && showtimes.length>0" @doubletap="selec" @pinchin="selec('in')"
        @pinchout="selec('out')" @panmove="touchMove" @panstart="touchMove" @panend="touchEnd" v-bind:pan-options="{ threshold: 1 }">
    ...
    <div class="selec" style="transform: scale(0.25);position:absolute;top:1.14rem;width:6rem;overflow:auto;transform-origin:0 0;    opacity: 0.9;z-index:120000">
      <v-touch tag="div" class="select-position" v-if="showtimes && showtimes.length>0" @pinchout="selec('out')" @panmove="touchMove"
        @panstart="touchMove" @panend="touchEnd" v-bind:pan-options="{ threshold: 1 }">
        <div class="position-num" v-bind:style="listPos.style">
          <div v-for="num in maxRow">{{num}}</div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      updateElementTransform() {
        ...
        this.blockPos.style = {
          webkitTransform: ['translate3d(' + (this.blockPos.translate.x) + 'px,' + (this.blockPos.translate.y) +
            'px,0)', 'scale(' + this.scale + ')'
          ].join(" "),
          translate: ['translate3d(' + (this.blockPos.translate.x) + 'px,' + (this.blockPos.translate.y) + 'px,0)',
            'scale(' + this.scale + ')'
          ].join(" "),
